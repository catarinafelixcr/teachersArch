    ...
    for commit in commits:
        try:
            ...
            commits_data.append({
                ...
                "last_minute": is_last_minute(detailed.created_at, deadline)
            })
    ...
    for pid in PROJECT_IDS:
        ...
        try:
            ...
            if start_str and end_str:
                ...
                end_str = (start_dt + timedelta(days=100)).isoformat()
            if not start_str or not end_str:
                print(f"⚠️  Projeto {pid} sem dados de tempo suficientes.")
                continue

            intervals = split_intervals(start_str, end_str, intervals=5)
            ...
            for i, (start, end) in enumerate(intervals):
                ...
                commits_cumulative.extend(commits)
                mrs_cumulative.extend(mrs)
                issues_cumulative.extend(issues)
                merges_to_main = count_merges_to_main(mrs_cumulative)
                metrics = aggregate_all_features(commits_cumulative, mrs_cumulative, issues_cumulative, branches_created, merges_to_main, group_id, interval=i+1)
